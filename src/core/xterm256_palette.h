...
//
// Alpha is intentionally not part of the model for the editor: colours are RGB only.
// The returned packed colour is always opaque (A=255). A value of 0 is reserved by the
// canvas as "unset" (theme default / transparent bg), so callers should use 0 only
// for that semantic, not as a valid xterm colour.
#pragma once
...
// Returns a packed 32-bit colour in Dear ImGui's IM_COL32 layout (ABGR, A=255).
// This is safe to store in AnsiCanvas::Colour32 and to cast to ImU32.
