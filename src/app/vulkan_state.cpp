...
void VulkanState::SetupVulkan(ImVector<const char*> instance_extensions)
{
    VkResult err;
    ...
    // Create Vulkan Instance
    {
        ...
        err = f_vkCreateDebugReportCallbackEXT(instance, &debug_report_ci, allocator, &debug_report);
        check_vk_result(err);
#endif
    }
...
void VulkanState::SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height)
{
    wd->Surface = surface;
    ...
    wd->PresentMode = ImGui_ImplVulkanH_SelectPresentMode(
        physical_device, wd->Surface, &present_modes[0], IM_ARRAYSIZE(present_modes));

    // Create SwapChain, RenderPass, Framebuffer, etc.
    IM_ASSERT(min_image_count >= 2);
    ImGui_ImplVulkanH_CreateOrResizeWindow(
        instance, physical_device, device, wd, queue_family, allocator,
        width, height, min_image_count, 0);
}

void VulkanState::ResizeMainWindow(ImGui_ImplVulkanH_Window* wd, int width, int height)
{
    ImGui_ImplVulkan_SetMinImageCount(min_image_count);
    ImGui_ImplVulkanH_CreateOrResizeWindow(
        instance, physical_device, device, wd,
...
void VulkanState::CleanupVulkan()
{
    ...
    f_vkDestroyDebugReportCallbackEXT(instance, debug_report, allocator);
#endif // APP_USE_VULKAN_DEBUG_REPORT
...
void VulkanState::CleanupVulkanWindow()
{
    ImGui_ImplVulkanH_DestroyWindow(instance, device, &main_window, allocator);
}

void VulkanState::FrameRender(ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data)
{
...
void VulkanState::FramePresent(ImGui_ImplVulkanH_Window* wd)
{
