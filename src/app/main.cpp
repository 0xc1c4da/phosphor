...
int main(int, char**)
{
    ...
    // The top of the stack is the currently active tool id.
    ...
    // Seed history with the initially active tool.
    sync_tool_stack();
    ...
    auto activate_prev_tool = [&]() {
        ...
        {
            // Pop current, activate previous (now at top).
            tool_stack.pop_back();
    ...
    while (!done)
    {
        ...
        {
            auto shortcut_for_action = [&](std::string_view action_id,
                                           std::string_view preferred_context) -> std::string
            {
            ...
            {
                const std::string sc_new = shortcut_for_action("app.file.new", "global");
                ...
                // Project IO + import/export (handled by IoManager).
                {
                    ...
                    io_manager.RenderFileMenu(window, file_dialogs, active_canvas, cbs, [&](std::string_view action_id) {
                        return shortcut_for_action(action_id, "global");
                    });
                }
                ...
                {
                    const std::string sc_e_ansi = shortcut_for_action("app.file.export_ansi", "global");
                    const std::string sc_e_png = shortcut_for_action("app.file.export_png", "global");
                    const std::string sc_e_utf8 = shortcut_for_action("app.file.export_utf8", "global");
                ...
                const std::string sc_quit = shortcut_for_action("app.quit", "global");
                ...
                ImGui::Separator();
                const std::string sc_settings = shortcut_for_action("app.settings.open", "global");
            ...
            if (ImGui::BeginMenu("Edit"))
            {
                ...
                const std::string sc_undo = shortcut_for_action("edit.undo", "editor");
                const std::string sc_redo = shortcut_for_action("edit.redo", "editor");
        ...
        // Keyboard shortcuts for Undo/Redo (only when a canvas is focused).
        {
            ...
            if (!any_popup)
            {
                ...
                {
                    // We only have a single "Image" tab today; PNG/APNG specifics are selected inside the dialog.
                    export_dialog.Open(ExportDialog::Tab::Image);
                }
                if (keybinds.ActionPressed("app.file.export_apng", kctx) && active_canvas)
                {
                    // APNG support isn't implemented yet; route to the Image export tab as the closest match.
                    export_dialog.Open(ExportDialog::Tab::Image);
                }
                ...
                {
                    // Prefer the focused canvas window, otherwise fall back to the active canvas window.
                ...
                {
                    // In-app "window" means the currently focused document/image window.
                    // Prefer focused canvas, else active canvas, else fall back to quitting.
