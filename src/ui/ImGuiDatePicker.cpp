// ImGuiDatePicker
// Copied from /references/ImGuiDatePicker/ImGuiDatePicker.cpp
// License: see /references/ImGuiDatePicker/LICENSE
//
// Local changes:
// - Linux portability: use gmtime_r/localtime_r when available
// - Avoid PushFont/PopFont with null altFont
    ...
    bool DatePickerEx(const std::string& label, tm& v, ImFont* altFont, bool clampToBorder, float itemSpacing)
    {
        ...
        if (comboOpen)
        {
            ...
            if (BeginTable(std::string("##Table_" + myLabel).c_str(), 7, TABLE_FLAGS, GetContentRegionAvail()))
            {
                ...
                const float cell_h = GetTextLineHeightWithSpacing() + 5.0f;
                for (int week = 1; week <= numWeeksInMonth; ++week)
                {
                    TableNextRow();
                    const auto days = CalendarWeek(week, firstDayOfMonth, numDaysInMonth);
                    for (int col = 0; col < 7; ++col)
                    {
                        TableSetColumnIndex(col);
                        const int day = days[col];
                        if (day != 0)
                        {
                            // Make cell IDs stable and unique even if labels repeat across instances.
                            PushID((week * 10) + col);
                            PushStyleVar(ImGuiStyleVar_FrameRounding, 20.0f);
                            ...
                            const float cell_w = GetContentRegionAvail().x;
                            if (Button(std::to_string(day).c_str(), ImVec2(cell_w, cell_h)))
                            {
